<!--
SIAGA-SDA DOCUMENT CONTROL
Project aktif: SIAGA-SDA
Dokumen ini adalah acuan pengembangan bertahap. Jangan melakukan penggantian nama aplikasi, jangan mengubah role/workflow/database/routing/auth tanpa instruksi eksplisit.
Codex wajib audit dan mapping sistem aktual sebelum coding.
-->

# SIAGA-SDA Logo Guideline

> Dokumen acuan SIAGA-SDA 2026  
> Instansi: Dinas Pekerjaan Umum — Bidang Sumber Daya Air — Kota Dumai  
> Prinsip: audit-safe, mobile-first, assignment-based, dan tidak rebuild total.


## Logo Resmi

Logo resmi disimpan di:

```text
/docs/assets/logo-siaga-sda.png
```

## Karakter Logo

Logo SIAGA-SDA memakai unsur:
- Tetesan air
- Shield/perisai
- Grafik monitoring
- Gelombang/aliran air
- Warna navy, biru, cyan, hijau

## Makna Logo

| Elemen | Makna |
|---|---|
| Tetesan air | Pengelolaan sumber daya air, banjir, drainase, hidrologi |
| Shield/perisai | Perlindungan, keamanan data, kontrol, pusat kendali |
| Grafik naik | Monitoring, analisis, progres, data real-time |
| Gelombang | Aliran air, konektivitas, gerak cepat |
| Navy | Stabilitas, pemerintahan, profesionalitas |
| Biru/Cyan | Air, informasi, monitoring |
| Hijau | Pembangunan, keberlanjutan, progres |

## Penggunaan

1. Logo tampil di halaman login.
2. Logo tampil pada sidebar/header.
3. Di bawah logo wajib ada kepanjangan:
   ```text
   Sistem Informasi, Analisis, Gerak Cepat dan Administrasi Sumber Daya Air
   ```
4. Jangan merusak proporsi logo.
5. Jangan mengubah warna logo di luar design system.
6. Gunakan versi kecil untuk sidebar collapse.

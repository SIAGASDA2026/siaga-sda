...
const MENU_GROUPS = [
  {
    ...
    items: [
      ...
      { href: '/administrasi', label: 'Administrasi', icon: FileText },
...
export function MobileNav() {
  ...
  const pendingApproval = projects.reduce((sum, project) => (
    sum +
  ...
  const getBadge = (href: string) => {
    ...
    if (href === '/chat' && totalChat > 0) return totalChat > 9 ? '9+' : String(totalChat)
  ...
  const filteredGroups = useMemo(() => {
    ...
    return MENU_GROUPS.map((group) => ({
      ...group,
      items: group.items
        .filter((item) => canAccessPage(currentUser?.role || 'pptk', item.href))
    ...
    <>
      {open && (
        <div className="fixed inset-0 z-[70] bg-slate-950/50 md:hidden" onClick={() => setOpen(false)}>
          <div
            className="absolute bottom-0 left-0 right-0 max-h-[88vh] overflow-hidden rounded-t-2xl bg-white shadow-2xl"
              ...
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-[11px] text-slate-400">{currentUser?.name || 'Pengguna'} - {getRoleLabel(currentUser?.role || 'pptk')}</div>
                  </div>
                </div>
                <button
                  type="button"
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/95 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur md:hidden">
        <div className="grid h-16 grid-cols-5">
          {PRIMARY_NAV.filter((item) => canAccessPage(currentUser?.role || 'pptk', item.href)).map((item) => {
            const Icon = item.icon
            const isActive = pathname === item.href || pathname.startsWith(`${item.href}/`)
            const badge = getBadge(item.href)

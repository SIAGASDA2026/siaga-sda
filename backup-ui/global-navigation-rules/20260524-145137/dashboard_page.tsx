          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5 2xl:grid-cols-7">
            {DASHBOARD_TABS.map((tab) => (
              <button
                ...
                className={`min-h-[62px] rounded-xl px-3 py-2.5 text-left transition ${activeTab === tab.id ? 'bg-[#0D2C54] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}
              >
                <div className="text-sm font-extrabold leading-tight">{tab.label}</div>
                <div className={`text-[10px] ${activeTab === tab.id ? 'text-white/70' : 'text-slate-400'}`}>{tab.desc}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        {activeTab === 'ringkasan' && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                { label: 'Total Paket', val: stats.total, icon: FolderOpen, color: 'bg-blue-50 text-blue-700', trend: null, sub: 'Filter aktif' },
                { label: 'On Track', val: stats.onTrack, icon: CheckCircle, color: 'bg-green-50 text-green-700', trend: 'up', sub: `${((stats.onTrack / Math.max(stats.total, 1)) * 100).toFixed(0)}% dari total` },
                { label: 'Warning', val: stats.warning, icon: AlertTriangle, color: 'bg-amber-50 text-amber-700', trend: null, sub: 'Perlu perhatian' },
                { label: 'Kritis', val: stats.kritis, icon: XCircle, color: 'bg-red-50 text-red-700', trend: 'down', sub: 'Butuh tindakan' },
              ].map((card) => {
                const Icon = card.icon
                return (
                  <div key={card.label} className="siaga-card p-4">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                { label: 'Total Anggaran', val: formatCurrency(stats.totalAnggaran), icon: BarChart2, bg: 'bg-slate-50', tone: 'text-slate-700' },
                { label: 'Masalah Open', val: stats.openMasalah, icon: AlertTriangle, bg: stats.openMasalah > 0 ? 'bg-red-50' : 'bg-green-50', tone: stats.openMasalah > 0 ? 'text-red-700' : 'text-green-700' },
                { label: 'Approval Pending', val: approvalPending, icon: ClipboardList, bg: approvalPending > 0 ? 'bg-amber-50' : 'bg-green-50', tone: approvalPending > 0 ? 'text-amber-700' : 'text-green-700' },
                { label: 'Paket Selesai', val: stats.selesai, icon: CheckCircle, bg: 'bg-blue-50', tone: 'text-blue-700' },
              ].map((item) => {
                const Icon = item.icon
                return (
                  <div key={item.label} className={`${item.bg} rounded-2xl border border-slate-100 p-4`}>
              ...
              {riskProjects.length === 0 ? (
                ...
                <div className="space-y-3">
                  {riskProjects.slice(0, 6).map((project) => {
                    ...
                    return (
                      <Link key={project.id} href={`/proyek/${project.id}`} className="block rounded-xl border border-slate-100 p-3 hover:bg-slate-50">
...
function SurveyInvestigationTab({ projects }: { projects: ReturnType<typeof useAppStore.getState>['projects'] }) {
  ...
  const latest = projects.flatMap((project) => project.surveys.map((survey) => ({ ...survey, projectName: project.nama, projectCode: project.kode }))).slice(0, 6)
    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
      <div className="siaga-card p-5 lg:col-span-2">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <MiniMetric label="Total Survey" value={totalSurvey} />
          <MiniMetric label="Menunggu Review" value={submitted} tone="amber" />
          <MiniMetric label="Disetujui" value={approved} tone="green" />
          <MiniMetric label="Dengan GPS" value={withGps} tone="blue" />
        </div>
      ...
      <div className="siaga-card p-5 lg:col-span-3">
        ...
        {latest.length === 0 ? (
          ...
          <div className="space-y-3">
            {latest.map((survey) => (
              <div key={survey.id} className="rounded-xl border border-slate-100 p-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <MiniMetric label="Paket Aktif" value={stats.total} tone="blue" />
        <MiniMetric label="On Track" value={stats.onTrack} tone="green" />
        <MiniMetric label="Warning" value={stats.warning} tone="amber" />
        <MiniMetric label="Kritis" value={stats.kritis} tone="red" />
      </div>
...
function WarningCenterTab({ stats, approvalPending, riskProjects }: { stats: any; approvalPending: number; riskProjects: ReturnType<typeof useAppStore.getState>['projects'] }) {
  const warningItems = [
    { label: 'Paket Kritis', value: stats.kritis, desc: 'Perlu eskalasi teknis', tone: 'red', icon: XCircle },
    { label: 'Paket Warning', value: stats.warning, desc: 'Pantau deviasi', tone: 'amber', icon: AlertTriangle },
    { label: 'Approval Pending', value: approvalPending, desc: 'Menghambat rekap sah', tone: 'blue', icon: ClipboardList },
    { label: 'Masalah Open', value: stats.openMasalah, desc: 'Perlu tindak lanjut', tone: 'red', icon: ShieldAlert },
  ]
...
function MiniMetric({ label, value, tone = 'slate' }: { label: string; value: number | string; tone?: 'slate' | 'blue' | 'green' | 'amber' | 'red' }) {
  const toneClass = {
              ...
              return (
                <tr key={project.id} className="transition-colors hover:bg-slate-50/50">
                  <td className="px-4 py-3">
                    <Link href={`/proyek/${project.id}`} className="block">
                  ...
                  <td className="px-3 py-3">
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${badge.bg} ${badge.text} ${badge.border}`}>
                      {badge.label}
                    </span>
                  </td>
                </tr>

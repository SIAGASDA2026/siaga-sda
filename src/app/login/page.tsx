...
export default function LoginPage() {
  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden bg-[#0D2C54] text-white">
      ...
      <main className="relative z-10 flex min-h-dvh items-center justify-center px-4 py-6 sm:py-8">
        <div className="w-full max-w-[600px]">
          <div className="mb-5 text-center sm:mb-8">
            <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-white p-2 shadow-2xl shadow-blue-950/30 sm:mb-5 sm:h-28 sm:w-28">
              <img src={BRAND.logoPath} alt={`Logo ${BRAND.name}`} className="h-16 w-16 object-contain sm:h-24 sm:w-24" loading="eager" />
            </div>
            <h1 className="text-3xl font-bold tracking-normal sm:text-4xl">{BRAND.name}</h1>
            <p className="mt-2 text-sm leading-relaxed text-blue-100 sm:text-lg">{BRAND.fullName}</p>
            <p className="mt-1 text-sm text-blue-200">{BRAND.agency} - {BRAND.unit} - {BRAND.city}</p>
          </div>

          <div className="rounded-2xl bg-white p-4 text-slate-900 shadow-2xl shadow-blue-950/30 sm:p-7 md:p-8">
            ...
            <div className="mb-5 grid grid-cols-2 gap-2 text-[11px] sm:text-xs md:grid-cols-4">
              {[

...
import { ArrowLeft, FileArchive, Plus, Search, Download, Upload, File, FileText, Image, MapPin, X } from 'lucide-react'
...
export default function DokumenPage() {
  ...
  const canManage = canAccess(currentUser?.role || 'pptk', 'upload_documents')
      ...
      <div className="p-5">
        {fromPaket && (
          <div className="mb-4 flex flex-wrap items-center gap-2">
            <Link href="/proyek" className="inline-flex items-center gap-2 rounded-xl border border-blue-100 bg-white px-4 py-2 text-sm font-bold text-blue-700 shadow-sm hover:bg-blue-50">
              <ArrowLeft className="h-4 w-4" />
              Kembali ke Paket Pekerjaan
            </Link>
            {initialProjectId !== 'all' && (
              <Link href={`/peta?proyek=${initialProjectId}&from=paket`} className="inline-flex items-center gap-2 rounded-xl border border-cyan-100 bg-cyan-50 px-4 py-2 text-sm font-bold text-cyan-700 hover:bg-cyan-100">

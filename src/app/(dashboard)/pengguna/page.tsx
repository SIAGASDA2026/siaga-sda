...
import { RoleAccessPanel } from '@/components/admin/RoleAccessPanel'
...
import { KeyRound, Link2, Users, Plus, Search, Shield, Eye, EyeOff } from 'lucide-react'
...
export default function PenggunaPage() {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'roles' | 'assignments'>('users')
  ...
  const canManageUser = (user: User) => {
    ...
    return canManageRole(currentUser.role, user.role)
  }
  ...
  return (
    <>
      ...
      <div className="p-5">
        <div className="mb-5 rounded-xl border border-slate-100 bg-white p-2">
          <div className="grid grid-cols-3 gap-2">
            {[
              { key: 'users' as const, label: 'Pengguna', icon: Users },
              { key: 'roles' as const, label: 'Role & Hak Akses', icon: KeyRound },
              { key: 'assignments' as const, label: 'Assignment', icon: Link2 },
            ].map((tab) => {
              const Icon = tab.icon
              ...
              return (
                <button
                  ...
                  className={`flex min-h-[44px] items-center justify-center gap-2 rounded-lg px-3 py-2 text-xs font-bold transition-colors md:text-sm ${
        ...
        {activeTab !== 'users' ? (
          <RoleAccessPanel users={users} projects={projects} currentUser={currentUser!} />
        ) : (
        <>
        {/* Stats per role */}
